...
<div class="container mt-4">
  <div class="card shadow-sm">
    ...
    <div class="d-flex flex-wrap justify-content-center mt-4">
      <a href="{{ route('sucursales.edit', $sucursal->id_branch) }}" class="btn btn-outline-warning btn-lg rounded mx-2 mb-2">
      ...
      <form action="{{ route('sucursales.destroy', $sucursal->id_branch) }}" method="POST" onsubmit="return confirm('¿Estás seguro de que deseas desactivar esta sucursal?');" class="mx-2 mb-2">
          @csrf
          @method('DELETE')
          <button type="submit" class="btn btn-outline-danger btn-lg rounded">
              <i class="fas fa-ban"></i> Desactivar
          </button>
      </form>

      <a href="{{ route('servicios.show', $sucursal->id_branch) }}" class="btn btn-outline-info btn-lg rounded mx-2 mb-2">
        <i class="fas fa-concierge-bell"></i> Servicios
      </a>
      <a href="{{ route('contratos.show', $sucursal->id_branch) }}" class="btn btn-outline-primary btn-lg rounded mx-2 mb-2">
        <i class="fas fa-file-contract"></i> Contratos
      </a>
      <a href="{{ url('informacion_contacto/' . $sucursal->id_branch) }}" class="btn btn-outline-secondary btn-lg rounded mx-2 mb-2">
        <i class="fas fa-address-book"></i> Información de Contacto
      </a>
      <a href="{{ url('trabajadores/' . $sucursal->id_branch) }}" class="btn btn-outline-dark btn-lg rounded mx-2 mb-2">
