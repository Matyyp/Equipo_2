...
<div class="container-fluid">
  <div class="card shadow-sm">
    ...
    <div class="card-body">
<form action="{{ route('maintenance.entries.update', ['maintenance' => $maintenance]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
          ...
          <select name="rental_car_id" id="rental_car_id" class="form-control" required>
            @foreach($cars as $car)
              <option value="{{ $car->id }}" {{ $maintenance->rental_car_id == $car->id ? 'selected' : '' }}>
        ...
        <div class="mb-3">
          ...
          <input type="number" name="scheduled_km" id="scheduled_km" class="form-control" value="{{ old('scheduled_km', $maintenance->scheduled_km) }}">
        </div>

        {{-- Fecha programada --}}
        <div class="mb-3">
          <label for="scheduled_date" class="form-label">Fecha programada</label>
          <input type="date" name="scheduled_date" id="scheduled_date" class="form-control" value="{{ old('scheduled_date', $maintenance->scheduled_date) }}">
        </div>
        ...
        <div class="mb-3">
          ...
          <select name="is_completed" id="is_completed" class="form-control">
            <option value="0" {{ !$maintenance->is_completed ? 'selected' : '' }}>No</option>
            <option value="1" {{ $maintenance->is_completed ? 'selected' : '' }}>Sí</option>
          </select>
        </div>
        ...
        <div class="mb-3">
          ...
          <input type="text" name="employee_name" id="employee_name" class="form-control" value="{{ old('employee_name', $maintenance->employee_name) }}">
        </div>

        {{-- Fecha finalización --}}
        <div class="mb-3">
          <label for="completed_date" class="form-label">Fecha de finalización</label>
          <input type="date" name="completed_date" id="completed_date" class="form-control" value="{{ old('completed_date', $maintenance->completed_date) }}">
        </div>
        ...
        <div class="mb-3">
          ...
          <input type="text" name="invoice_number" id="invoice_number" class="form-control" value="{{ old('invoice_number', $maintenance->invoice_number) }}">
        </div>

        {{-- Imagen actual --}}
        @if($maintenance->images->first())
        <div class="mb-3">
          ...
          <div class="text-center mb-2">
            <div class="border rounded overflow-hidden mb-1" style="width:100px; height:70px;">
              <img src="{{ tenant_asset($maintenance->images->first()->image_path) }}" alt="Factura" class="w-100 h-100 object-cover" onerror="this.onerror=null;this.src='https://via.placeholder.com/100x70?text=No+Imagen';">
            </div>
